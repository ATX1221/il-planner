"""Multi-mode transformer planner.

Encodes ego, agents and map as tokens, then emits `num_modes` trajectories plus a mode
logit. Trained with winner-take-all (il_planner/objectives/wta_objective.py).

With `use_ego_history=False` the ego token comes from the present state only, via
...
class TransformerPlanner(TorchModuleWrapper):
    def __init__(
        ...
        self.multimodal_proj = nn.Linear(hidden_size, self.num_modes*hidden_size)
        self.loc = mlp(hidden_size,self.future_steps*self.trajectory_state_size, 2 * hidden_size)
        self.pi = mlp(hidden_size, 1, 2 * hidden_size)

        # Auxiliary head: predict every OTHER agent's future. Never used at inference -
        # it exists to supervise the shared encoder. One ego trajectory per sample is a
        # thin gradient signal; ~32 agents per sample is a thick one, and it forces the
        # agent tokens to encode where traffic is GOING rather than only where it is.
        # That is exactly the cue the mode classifier lost when ego history was dropped.
        self.predict_agents = predict_agents
        self._agent_predictor = (
            mlp(hidden_size, self.future_steps * 2, 2 * hidden_size) if predict_agents else None
        )
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        modes = self.multimodal_proj(ego_cmp) 
        modes = modes.reshape(modes.shape[0],self.num_modes,self.hidden_size)   # (B, K, H)

        loc = self.loc(modes)
        loc = loc.reshape(loc.shape[0], self.num_modes, self.future_steps, self.trajectory_state_size) # (B , K , T, C)


        pi = self.pi(modes).squeeze(-1) # (B, K)
        best = pi.argmax(dim=-1)
        selected = loc[torch.arange(loc.shape[0], device=loc.device), best] # (B,T,C)

        out = {
            "trajectory": Trajectory(data=selected),
            "modes": loc,
            "probability": pi,
                }

        if self._agent_predictor is not None:
            # x[:, 1:N] - the AGENT tokens after the encoder, ego (token 0) excluded and
            # the lane tokens after index N excluded. Predicting ego here would duplicate
            # what the trajectory decoder already does.
            n_agents = agents.shape[1]
            out["agent_prediction"] = self._agent_predictor(x[:, 1:n_agents]).view(
                x.shape[0], n_agents - 1, self.future_steps, 2
            )                                                       # (B, N-1, T_future, 2)
            # The supervision lives in the FEATURE, and an objective only receives
            # (predictions, targets) - so hand it through here. Absent during simulation,
            # where there is no future to read; the objective is not run there either.
            if "target" in agent:
                out["agent_target"] = agent["target"]
                out["agent_target_valid_mask"] = agent["target_valid_mask"]
        return out
